...
with open(r'C:\Users\leticia\Documents\GitHub\Abaqus_WELL_\wellClosure_planestrain.json') as f:
    data = json.load(f)

def process_lithology(data, global_top, global_bottom):
    # 1. Pega os limites globais do modelo (Ex: 2500)
    ...
    for layer in lithology:
        l_top = layer["Top"] # 3600
        l_bottom = layer["Bottom"] #4200
        
        # Ignora as rochas que estão completamente fora do domínio do poço
        if l_depth:
            if l_bottom <= global_top and l_top >= global_bottom:
                continue
